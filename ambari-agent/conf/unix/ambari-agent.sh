...
export AMBARI_AGENT_CONF_DIR=conf

# Because Ambari rpm unpacks modules here on all systems
export PYTHONPATH=sbin:$PYTHONPATH

AMBARI_AGENT=ambari-agent
export AMBARI_PID_DIR=/var/run/ambari-agent
PIDFILE=$AMBARI_PID_DIR/$AMBARI_AGENT.pid
export AMBARI_AGENT_LOG_DIR=logs
mkdir -p $AMBARI_AGENT_LOG_DIR
OUTFILE=$AMBARI_AGENT_LOG_DIR/ambari-agent.out
touch $OUTFILE
LOGFILE=$AMBARI_AGENT_LOG_DIR/ambari-agent.log
export AGENT_SCRIPT=sbin/ambari_agent/main.py
export AMBARI_AGENT_PY_SCRIPT=sbin/ambari_agent/AmbariAgent.py
OK=1
NOTOK=0
...
change_files_permissions() {
	...
	./ambari-sudo.sh chown -R $current_user "logs/"
	./ambari-sudo.sh chown -R $current_user "data/"
	./ambari-sudo.sh chown -R $current_user "cache/"
	./ambari-sudo.sh chown 	$current_user "./"
}
...
if [ -a ambari-env.sh ]; then
  ./ambari-sudo.sh chown -R $current_user "./ambari-env.sh"
  ./ambari-env.sh
fi
...
case "$1" in
  ...
  *)
        tput bold
        echo "Usage: ambari-agent.sh {start|stop|restart|status|reset <server_hostname>}"
        tput sgr0
        retcode=1
esac
